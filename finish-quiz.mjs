...
function xpForResult(correct, total) {
  if (total <= 0) return 0;
  const XP_POR_ACERTO = 22;
  const BONUS_CONCLUSAO = 10;
  const percent = correct / total;
  let bonusDesempenho = 0;
  if (percent >= 1) bonusDesempenho = 40;
  else if (percent >= 0.8) bonusDesempenho = 20;
  else if (percent >= 0.6) bonusDesempenho = 10;
  return BONUS_CONCLUSAO + correct * XP_POR_ACERTO + bonusDesempenho;
}
