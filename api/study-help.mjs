...
CURIOSIDADE: <uma única frase curta (máximo 30 palavras) com um fato curioso, surpreendente ou uma dica de prova sobre o assunto — algo que prenda a atenção e ajude a fixar o conteúdo. Não repita algo já dito no resumo.>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const curiosidadeMatch = raw.match(/CURIOSIDADE:\s*(.+?)\s*\n\s*EXPLICACAO:/i);
    ...
    const keywords = keywordsMatch
      ...
      : [];
    const funFact = curiosidadeMatch ? curiosidadeMatch[1].trim() : '';
    const explanation = explicacaoMatch ? explicacaoMatch[1].trim() : raw;

    return res.status(200).json({ subject: correctedSubject, topic: cleanTopic, summary, keywords, funFact, explanation });
  } catch (error) {
